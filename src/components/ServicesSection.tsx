        ...
        <motion.div className="relative mt-16 overflow-hidden rounded-[2.5rem] shadow-2xl" initial={{ opacity: 0, y: 60, scale: 0.96 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.8, delay: 0.2 }}>
          <div className="relative h-[520px] md:h-[620px]">
            ...
            <div className="absolute right-6 top-6 flex gap-2 md:right-10 md:top-10">
              <button onClick={prev} aria-label="Previous" className="flex h-12 w-12 items-center justify-center rounded-full border border-white/30 bg-white/10 text-white backdrop-blur transition hover:bg-white/20">
                <ChevronLeft className="h-5 w-5" />
              </button>
              <button onClick={next} aria-label="Next" className="flex h-12 w-12 items-center justify-center rounded-full border border-white/30 bg-white/10 text-white backdrop-blur transition hover:bg-white/20">
            ...
            <div className="relative z-10 flex h-full max-w-2xl flex-col justify-center p-8 pb-32 md:p-16 md:pb-40">
              <AnimatePresence mode="wait">
                <motion.div key={current.number} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.4 }}>
                  <span className="inline-flex items-center rounded-full border border-white/30 px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.25em] text-white/90">{current.number}</span>
                  <h3 className="mt-6 text-4xl font-bold leading-[1.05] tracking-tight text-white md:text-6xl">{current.title}</h3>
                  <p className="mt-6 max-w-lg text-base text-white/80 md:text-lg">{current.description}</p>
                </motion.div>
              </AnimatePresence>
            </div>

            <div className="absolute inset-x-0 bottom-0 z-20 p-4 md:p-6">
              <div className="grid gap-3 md:gap-4" style={{ gridTemplateColumns: `repeat(${Math.min(services.length, 5)}, minmax(0, 1fr))` }}>
                {services.map((s, i) => {
                  const isActive = i === active;
                  return (
                    <button key={i} onClick={() => setActive(i)} className="group relative h-20 overflow-hidden rounded-2xl text-left transition md:h-24" style={{ boxShadow: isActive ? "0 0 0 2px var(--brand-green)" : "0 0 0 1px rgba(255,255,255,0.2)" }}>
                      ...
                      <span className="absolute inset-x-0 bottom-0 p-2.5 text-[11px] font-semibold leading-tight text-white md:p-3 md:text-sm">{s.title}</span>
                    </button>
                  );
                })}
              </div>
