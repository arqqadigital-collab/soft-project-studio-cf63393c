...
import { useKSAComplianceContent } from "@/lib/ksaComplianceContent";

const ICONS: Record<string, LucideIcon> = {
  Building2, Hospital, Share2, Database, Globe2, FileBadge, MapPin,
  ShieldCheck, Languages, Layers, Settings, GraduationCap, RefreshCw,
};
const Ic = (name?: string) => (name && ICONS[name]) || ShieldCheck;
...
export default function KSACompliance() {
  const c = useKSAComplianceContent();
  const v = c._visible;
      ...
      <main className="pt-20 relative min-h-[90vh] w-full overflow-hidden bg-background">
        <div className="absolute inset-0">
          {c.Hero.mediaKind === "video" ? (
            ...
            <img src={c.Hero.mediaUrl} alt="" className="h-full w-full object-cover" />
          )}
            ...
            >
              ...
              <div className="mt-10 flex flex-wrap items-center justify-center gap-2">
                {c.Hero.chips.map((chip: string) => (
                  <span key={chip} className="rounded-full border border-white/15 bg-white/5 px-3 py-1.5 text-xs text-white/80 backdrop-blur">
                    {chip}
                  </span>
                ))}
              </div>
      ...
      <section className="relative overflow-hidden px-6 py-24 md:px-12" style={{ backgroundColor: "#091628" }}>
        ...
        <div className="relative mx-auto max-w-6xl">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <p className="mt-4 text-sm text-white/65 md:text-base">{c.Outcomes.body}</p>
          </div>
            ...
            {c.FAQ.items.map((f: any) => (
              <details key={f.question} className="group rounded-2xl border border-border bg-card p-6 transition-shadow open:shadow-[var(--shadow-brand)]">
                <summary className="flex cursor-pointer list-none items-start justify-between gap-4 text-start">
                  ...
                  <span className="mt-1 inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-border text-foreground/60 transition-transform group-open:rotate-45" aria-hidden>
                    +
                  </span>
                </summary>
      ...
      <section id="contact" className="relative overflow-hidden px-6 py-24 md:px-12" style={{ backgroundColor: "#091628" }}>
        <div className="absolute inset-0">
          {c["Final CTA"].mediaKind === "video" ? (
            ...
            <img src={c["Final CTA"].mediaUrl} alt="" className="absolute inset-0 h-full w-full object-cover" />
          )}
        ...
        <div className="relative mx-auto max-w-4xl text-center">
          ...
          <p className="mt-8 text-lg leading-relaxed text-white/80">{c["Final CTA"].body}</p>

          <div className="mt-12 flex flex-col items-center justify-center gap-4 sm:flex-row">
            <a href={c["Final CTA"].ctaHref} className="inline-flex items-center gap-3 rounded-full px-6 py-3 text-sm font-semibold sm:px-10 sm:py-5 sm:text-base text-white shadow-[var(--shadow-brand)] transition-transform hover:scale-105" style={{ background: "var(--gradient-brand)" }}>
              {c["Final CTA"].ctaLabel} <ArrowRight className="h-5 w-5" />
            </a>
            <a href={c["Final CTA"].ctaHref2} className="inline-flex items-center gap-3 rounded-full border border-white/30 bg-white/5 px-6 py-3 text-sm font-semibold sm:px-10 sm:py-5 sm:text-base text-white backdrop-blur transition-colors hover:bg-white/15">
