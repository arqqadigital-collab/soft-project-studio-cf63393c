...
const faqs = [
  { q: "How is legacy PACS data migrated to Secreta PACS?", a: "A migration assessment profiles every legacy archive — image volume, modality distribution, date range, and storage format. Active and recent studies are prioritized while the full historical archive migrates in the background, in parallel with your existing PACS, until cutover. Every migrated study is validated for completeness and integrity before the legacy system is retired." },
  { q: "What is the storage architecture and how is data protected against loss?", a: "Secreta PACS uses a tiered storage architecture with automatic replication across geographically separated nodes. Primary storage holds active studies. Near-line storage holds less-frequently accessed studies. Deep archive holds long-term retention. Every tier replicates automatically — no single hardware failure can result in image loss. Annual disaster recovery testing is included in enterprise SLA agreements." },
  { q: "Can radiologists use their own diagnostic monitors for remote reading?", a: "Yes. Secreta PACS delivers images calibrated to DICOM GSDF standards through the browser-based viewer — calibration is applied by the viewer, not the workstation. Radiologists can read at diagnostic quality from any compliant monitor at home or in a remote office." },
  { q: "What happens to image availability during planned maintenance?", a: "Planned maintenance runs in low-activity windows — typically overnight — and is communicated in advance. During maintenance, PACS operates in read-only mode so image viewing continues. Our enterprise SLA guarantees 99.9% monthly availability." },
  { q: "How long are images retained in the archive?", a: "Retention is configurable by study type, patient age group, and regulatory requirement. Standard configuration follows your jurisdiction — typically 7–10 years for adults and until age 25 (or longer) for pediatric studies. Medicolegal and research holds are managed through a structured exception workflow." },
  { q: "Does the system support 3D post-processing and advanced visualization?", a: "Yes. Multiplanar reconstruction, maximum intensity projection, volume rendering, 3D reconstruction, and fusion imaging are available directly in the viewer — without switching to a separate workstation application." },
  { q: "Can the system route studies to AI applications for analysis?", a: "Yes. Multiple AI applications can be connected simultaneously, with study routing rules configured by modality, body region, and clinical indication. Results are returned to the viewer and the report workflow." },
];
