...
const ICONS: Record<string, any> = {
  ...
  Network, ShieldCheck, Zap, Gauge, LineChart, Map: MapIcon, Settings2, Workflow, TrendingUp,
};

export default function Logistics() {
  const c = useLogisticsContent();
  const v = c._visible;
            ...
            <div className="flex items-center px-6 py-20 md:px-12 md:py-28">
              <div className="mx-auto max-w-xl">
                <h3 className="text-3xl font-bold leading-tight text-foreground md:text-4xl">
                  {c["Traditional Fail"].heading}
                </h3>
                <p className="mt-5 text-base leading-relaxed text-muted-foreground">
                  {c["Traditional Fail"].body}
                </p>
          ...
          <div className="mx-auto max-w-6xl">
            ...
            <div className="mt-14 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
              {c.Capabilities.items.map((cap: any) => {
                const Icon = ICONS[cap.icon] ?? Warehouse;
                return (
                  <motion.div
                    key={cap.title}
          ...
          <div className="mx-auto max-w-6xl">
            ...
            <div className="mt-14 space-y-5">
              {c.Implementation.items.map((p: any, idx: number) => {
                const Icon = ICONS[p.icon] ?? MapIcon;
                return (
                  <motion.div
                    key={p.n}
