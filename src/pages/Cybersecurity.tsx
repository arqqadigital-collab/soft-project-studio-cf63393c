...
const ICONS: Record<string, LucideIcon> = {
  ...
  Award, Handshake, Building2, Workflow, BarChart3,
};
const Icon = (name?: string) => ICONS[name ?? ""] ?? ShieldCheck;
...
function StatsCounter({ items }: { items: Array<{ prefix?: string; value: number; decimals?: number; suffix: string; label: string }> }) {
...
export default function Cybersecurity() {
  const c = useCybersecurityContent();
  const v = c._visible;
  return (
    <>
      {v.Hero && (
        <main className="relative min-h-[90vh] w-full overflow-hidden bg-background pt-20">
          <div className="absolute inset-0">
            ...
            <div
              ...
              style={{ background: "linear-gradient(180deg, rgba(5,11,24,0.85) 0%, rgba(7,20,43,0.72) 55%, rgba(5,11,24,0.95) 100%)" }}
            />
          </div>
            ...
            <div className="mt-14 grid grid-cols-1 gap-6 md:grid-cols-3">
              {c["The Problem"].items.map((p) => (
                <div key={p.n} className="group overflow-hidden rounded-3xl border border-white/10 bg-white/[0.03] transition-all hover:-translate-y-1 hover:border-white/20">
                  ...
                  <div className="p-6">
                    <p className="text-xs font-semibold uppercase tracking-[0.25em] text-red-400">{p.n} — Threat</p>
        ...
        <section className="relative bg-background px-6 py-24 md:px-12 md:py-32">
          <div className="mx-auto max-w-6xl">
            <div className="text-center">
              ...
              <p className="mx-auto mt-6 max-w-3xl text-base leading-relaxed text-muted-foreground md:text-lg">{c.Pillars.body}</p>
            </div>
            <div className="mt-14 grid grid-cols-1 gap-6 md:grid-cols-3">
              {c.Pillars.items.map((p) => {
                const Ic = Icon(p.icon);
                return (
                  <motion.div key={p.n} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.5 }} className="rounded-3xl border border-border bg-card p-7">
                    ...
                    <p className="mt-6 text-xs font-semibold uppercase tracking-[0.25em]" style={{ color: "var(--brand-blue)" }}>Pillar {p.n} — {p.label}</p>
