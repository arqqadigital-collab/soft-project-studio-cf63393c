...
import { useRisContent } from "@/lib/risContent";
...
function ExpandingJourney({
  ...
  return (
    <div className="mt-14 flex flex-col gap-3 md:h-[520px] md:flex-row md:gap-4">
      {steps.map((step, i) => {
        const Icon = (step.icon && ICONS[step.icon]) || Inbox;
...
export default function RIS() {
  ...
  const integrationLogos = (integrations.groups ?? []).flatMap((g: any) => g.items ?? []);
          ...
          <div className="mx-auto max-w-6xl">
            ...
            <div className="mt-14 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {platform.items.map((f: any, i: number) => {
                const Icon = (f.icon && ICONS[f.icon]) || CalendarClock;
                return (
                  <motion.div
                    key={f.title}
