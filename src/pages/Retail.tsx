...
import { useRetailContent } from "@/lib/retailContent";

const ICONS: Record<string, LucideIcon> = {
  ShoppingBag, Layers, Boxes, Activity, Calculator, ScanSearch, Settings2, Workflow, Rocket, TrendingUp,
...
const Ico = ({ name, className }: { name?: string; className?: string }) => {
  const C = (name && ICONS[name]) || ShoppingBag;
  return <C className={className} />;
};

export default function Retail() {
  const c = useRetailContent();
  const v = c._visible;
      ...
      {v.Hero && (
        <main className="relative min-h-[90vh] w-full overflow-hidden bg-background pt-20">
          <div className="absolute inset-0">
            <img src={c.Hero.image} alt="Modern omnichannel retail store" className="absolute inset-0 h-full w-full object-cover" />
        ...
        <section className="relative bg-background">
          <div className="grid min-h-[600px] grid-cols-1 md:grid-cols-2">
            ...
            <div className="relative h-80 min-h-full md:h-auto">
              <img src={c["Traditional Fail"].image} alt="Disconnected retail systems" loading="lazy" className="absolute inset-0 h-full w-full object-cover" />
            </div>
          </div>
        </section>
      )}
