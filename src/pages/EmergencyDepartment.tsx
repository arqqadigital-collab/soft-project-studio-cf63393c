...
import { useEdContent } from "@/lib/edContent";

const ICONS: Record<string, LucideIcon> = {
  Ambulance, ClipboardList, Activity, Stethoscope, Timer, BedDouble,
  FileText, HeartPulse, Pill, Radio, BarChart3, Users, Siren,
};
...
function ExpandingJourney({ steps }: { steps: Array<{ icon?: string; title: string; image?: string; description?: string }> }) {
  ...
  return (
    <div className="mt-14 flex flex-col gap-3 md:flex-row md:gap-4 md:h-[520px]">
      {steps.map((step, i) => {
        const Icon = (step.icon && ICONS[step.icon]) || Siren;
          ...
          >
            ...
            <div className={`relative flex h-full min-h-[320px] flex-col ${isActive ? "p-7" : "p-4"}`}>
              ...
              <div className="mt-6 flex h-[calc(100%-3.5rem)] flex-col">
                <motion.div animate={{ opacity: isActive ? 1 : 0 }} transition={{ duration: 0.3, delay: isActive ? 0.25 : 0 }} className="flex-1">
                  {isActive && (
                    <>
            ...
            <section className="flex flex-1 items-center justify-center px-6 pb-28 pt-4 md:px-12">
              <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="mx-auto flex max-w-5xl flex-col items-center text-center">
                ...
                <div className="mt-9 flex flex-col items-center gap-3 sm:flex-row">
                  {hero.ctaLabel && (
                    <a href={hero.ctaHref || "#contact"} className="inline-flex items-center gap-2 rounded-full px-8 py-4 text-sm font-semibold text-white shadow-[var(--shadow-brand)] transition-transform hover:scale-105" style={{ background: "var(--gradient-brand)" }}>
                      {hero.ctaLabel} <ArrowRight className="h-4 w-4" />
                    </a>
                  )}
                  {hero.ctaLabel2 && (
                    <a href={hero.ctaHref2 || "#contact"} className="inline-flex items-center gap-2 rounded-full border border-white/30 bg-white/5 px-8 py-4 text-sm font-semibold text-white backdrop-blur transition-colors hover:bg-white/15">
                      {hero.ctaLabel2}
                    </a>
                  )}
                </div>
        ...
        <section id="contact" className="relative overflow-hidden px-6 py-24 md:px-12" style={{ backgroundColor: "#091628" }}>
          ...
          <div className="relative mx-auto max-w-4xl text-center">
            ...
            <div className="mt-12 flex flex-col items-center justify-center gap-4 sm:flex-row">
              {cta.primaryLabel && (
                <a href={cta.primaryHref || "#"} className="inline-flex items-center gap-3 rounded-full px-10 py-5 text-base font-semibold text-white shadow-[var(--shadow-brand)] transition-transform hover:scale-105" style={{ background: "var(--gradient-brand)" }}>
                  {cta.primaryLabel} <ArrowRight className="h-5 w-5" />
                </a>
              )}
              {cta.secondaryLabel && (
                <a href={cta.secondaryHref || "#"} className="inline-flex items-center gap-3 rounded-full border border-white/30 bg-white/5 px-10 py-5 text-base font-semibold text-white backdrop-blur transition-colors hover:bg-white/15">
                  {cta.secondaryLabel}
                </a>
              )}
            </div>
