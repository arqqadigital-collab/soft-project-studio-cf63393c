...
export default function About() {
  ...
  const mv = c["Mission & Vision"];
  const journey = c["Our Journey"];
  const cta = c["Final CTA"];

  return (
    <>
      <SeoHead
        title="About SBS — Enterprise & Healthcare Technology"
        description="SBS is a specialised provider of enterprise software solutions, healthcare technologies, and digital transformation services."
        ogType="website"
      />
      ...
      {(v.Introduction || v["Mission & Vision"]) && (
        <section className="relative z-20 -mt-16 rounded-t-[2.5rem] bg-white px-6 pb-24 pt-24 shadow-[0_-20px_60px_-20px_rgba(0,0,0,0.4)] md:px-12">
          <div className="mx-auto max-w-6xl">
            ...
            {v["Mission & Vision"] && Array.isArray(mv.items) && mv.items.length > 0 && (
              <div className="mt-16 grid gap-8 md:grid-cols-2">
                {mv.items.map((item: any, i: number) => {
      ...
      {/* Our Journey */}
      {v["Our Journey"] && Array.isArray(journey.items) && journey.items.length > 0 && (
        <section
            ...
            <div className="relative mt-20">
              ...
              <div className="space-y-12">
                {journey.items.map((step: any, i: number) => {
                  const Icon = iconFor(step.icon, Building2);
                  const isRight = i % 2 === 1;
                  return (
                    <motion.div
                      key={`${step.title}-${i}`}
      ...
      {v["Final CTA"] && (
        ...
        >
          <div className="relative mx-auto max-w-4xl text-center">
            ...
            {cta.body && (
              <p className="mt-8 text-lg leading-relaxed text-white/80">{cta.body}</p>
            )}
            {cta.body2 && (
