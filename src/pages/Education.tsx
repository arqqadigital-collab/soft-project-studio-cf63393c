...
const ICONS: Record<string, LucideIcon> = {
  ...
  Workflow, Rocket, TrendingUp, Cpu, RefreshCw, DollarSign, Network,
  ShieldCheck, Zap, Recycle, LineChart, Eye, Target,
};
const Ico = (n: string) => ICONS[n] ?? CheckCircle2;

export default function Education() {
  const c = useEducationContent();
  const v = c._visible;
      ...
      <section className="relative bg-background">
        <div className="grid min-h-[600px] grid-cols-1 md:grid-cols-2">
          ...
          <div className="relative h-80 min-h-full md:h-auto">
            <img src={c["Traditional Fail"].image} alt="Disconnected academic systems" loading="lazy" width={1024} height={1024} className="absolute inset-0 h-full w-full object-cover" />
          </div>
        </div>
      </section>
      )}
