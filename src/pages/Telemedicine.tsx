...
import { useTelemedicineContent } from "@/lib/telemedicineContent";
import { useHorizontalScroll } from "@/hooks/use-horizontal-scroll";

const iconFor = (name?: string) => (name && (LucideIcons as any)[name]) || LucideIcons.Video;
...
export default function Telemedicine() {
  ...
  const content = useTelemedicineContent();
    ...
    <>
              ...
              {(problem.items || []).map((card: any, i: number) => (
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col p-6 md:p-7">
                    ...
                    <p className="mt-3 text-sm leading-relaxed text-white/75">{card.description || card.body}</p>
                  </div>
                </article>
              ))}
            </motion.div>
            ...
            {(integrations.groups || []).map((group: any) => {
              ...
              return (
                <div key={group.title} className="rounded-2xl border border-border bg-card p-7">
                  ...
                  {group.subtitle && (
                    <p className="mt-1 text-xs uppercase tracking-wider text-foreground/60">{group.subtitle}</p>
                  )}
                  <ul className="mt-4 flex flex-wrap gap-2">
                    {(group.items || []).map((it: string) => (
                      <li
                        key={it}
                        className="rounded-full border border-border bg-background/70 px-3 py-1 text-xs font-medium text-foreground/80"
                      >
                        {it}
                      </li>
                    ))}
      ...
      <section id="contact" className="relative overflow-hidden px-6 py-24 md:px-12" style={{ backgroundColor: "#091628" }}>
        ...
        <div className="relative mx-auto max-w-4xl text-center">
          ...
          <p className="mt-8 text-lg leading-relaxed text-white/85">{finalCta.body}</p>
          ...
          {finalCta.footnote && (
            <p className="mt-8 text-sm italic text-white/70">{finalCta.footnote}</p>
          )}
        </div>
      </section>
      )}
