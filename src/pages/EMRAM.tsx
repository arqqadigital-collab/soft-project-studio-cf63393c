...
import { useEMRAMContent } from "@/lib/emramContent";

const ICONS: Record<string, LucideIcon> = {
  ClipboardList, Map, Workflow, Pill, Brain, Database, Sparkles, BadgeCheck,
  Users, GitBranch, Target, Network, ShieldCheck, AlertTriangle, CheckCircle2,
};
const Icon = (name?: string) => ICONS[name ?? ""] ?? ClipboardList;
      ...
      {steps.map((step: any, i: number) => {
        ...
        return (
          ...
          >
            ...
            <div className={`relative flex h-full min-h-[320px] flex-col ${showExpanded ? "p-6" : "p-3"}`}>
              <div
                ...
                >
                  {showExpanded && (
                    <>
                      ...
                      <p className="mt-3 max-w-md text-xs leading-relaxed text-white/85 md:text-sm">{step.description}</p>
                    </>
                  )}
                </motion.div>
                {!showExpanded && (
            ...
            <div className="mt-12 grid gap-6 md:grid-cols-2">
              {c.Integrations.groups.map((g: any) => (
                <div key={g.title} className="rounded-3xl border border-border bg-card p-8 shadow-sm">
                  ...
                  {g.subtitle && (
                    <p className="mt-1 text-xs text-foreground/60">{g.subtitle}</p>
                  )}
